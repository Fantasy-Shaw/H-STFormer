...
class IncrementalDataGenerator:
    def __init__(self,
                 ...
                 outPutDirPrefix: str = os.path.join("..", r"raw_data\__PreProcessOutput\tmp"),
        ...
        self.outPutDir = outPutDirPrefix
        self.stage_1_workdir = outPutDirPrefix + "Stage1"
        self.stage_2_workdir = outPutDirPrefix + "Stage2"
        for _ in [self.stage_1_workdir, self.stage_2_workdir]:
    ...
    def genSpatialTemporalSplitDataset(self):
        ...
        # # [Deprecated]Spatial-Temporal shrink
        ...
        dyna_stage_2.drop([i for i, t in enumerate(dyna_stage_2.time) if
                           datetime.strptime(t, "%Y-%m-%dT%H:%M:%SZ") <= self.timeBoundary], inplace=True)
        # Spatial shrink, ~xxx[yy].isin(zzz) means "is not in"
        dyna_stage_1.loc[~dyna_stage_1["entity_id"].isin(baseIndices), ["traffic_flow"]] = 0
        dyna_stage_2.loc[~dyna_stage_2["entity_id"].isin(baseIndices), ["traffic_flow"]] = 0
        # Reset Idx
        ...
        # Geo is not modified, shrink spatial feature by modifying reo file.
        ...
        del dyna_stage_1, rel_stage_1, dyna_stage_2, rel_stage_2
        gc.collect()
    ...
    def genTemporalSplitDataset(self):
        ...
        shutil.copy(self.configFile, self.stage_2_workdir)
